...
local function create_world()
	...
	for i = 1, NUMBODIES do
		...
		local mass = ode.mass_box(1, randomf() + 0.1, randomf() + 0.1, randomf() + 0.1, randomf() + 1)
	...
	for i = 1, NUMBODIES do
		...
		body[i]:set_position(pos + randomvec3(0.1))
	end
end
...
local function run_test(stepfunc)
	create_world()
	local step
	if stepfunc == "step" then
		step = world.step
	elseif stepfunc == "quick_step" then
		step = world.quick_step
	else
