...
local function FAILED(i)
	COUNTER[i] = COUNTER[i] and COUNTER[i] + 1 or 1
end
...
local function test_box_point_depth()
	...
	space:add(box)
	-- make a random box
	local s = vec3(random() + 0.1, random() + 0.1, random() + 0.1)
	local C = randomvec3(1.0)
	...
	if abs(box:point_depth(C) - ss / 2) > TOL then
		FAILED(1)
	end
	-- test that points on surface have depth 0
	local P = vec3(randomf(-0.5, 0.5) * s.x, randomf(-0.5, 0.5) * s.y, randomf(-0.5, 0.5) * s.z)
	local i = random(1, 3)
	P[i] = randomsign() * s[i] * 0.5
	P = C + box:get_rotation() * P
	if abs(box:point_depth(P)) > TOL then
		FAILED(2)
	end
	-- test that points outside the box have negative depth
	local P = vec3(
		randomsign() * (s.x / 2 + random() + 0.01),
		randomsign() * (s.y / 2 + random() + 0.01),
		randomsign() * (s.z / 2 + random() + 0.01)
	)
	P = C + box:get_rotation() * P
	if box:point_depth(P) >= 0 then
		FAILED(3)
	end
	-- test that points inside the box have positive depth
	local P = vec3(s.x * 0.99 * randomf(-0.5, 0.5), s.y * 0.99 * randomf(-0.5, 0.5), s.z * 0.99 * randomf(-0.5, 0.5))
	P = C + box:get_rotation() * P
	if box:point_depth(P) <= 0 then
		FAILED(4)
	end
	...
	P = C + box:get_rotation() * P
	if abs(box:point_depth(P) - d) >= TOL then
		FAILED(5)
	end
end
...
local function test_capsule_point_depth()
	...
	local P = randomf(-0.5, 0.5) * l * vec3(0, 0, 1)
	P = C + capsule:get_rotation() * P
	if abs(capsule:point_depth(P) - r) >= TOL then
		FAILED(1)
	end
	-- test that a point on the surface (excluding caps) has depth 0
	local beta = randomf(0, 2 * pi)
	local P = vec3(r * sin(beta), r * cos(beta), randomf(-0.5, 0.5) * l)
	P = C + capsule:get_rotation() * P
	if abs(capsule:point_depth(P)) > TOL then
		FAILED(2)
	end
	-- test that a point on a surface of a cap has depth 0
	local dir = randomdir()
	local zofs = vec3(0, 0, l / 2)
	if dir * vec3(0, 0, 1) < 0 then
		zofs = -zofs
	end
	...
	local P = C + capsule:get_rotation() * (zofs + 0.99 * r * dir)
	if capsule:point_depth(P) < 0 then
		FAILED(4)
	end
	-- test point depth of a point inside the cylinder
	local beta = randomf(0, 2 * pi)
	local d = randomf(-1, 1) * r
	local P = vec3((r - d) * sin(beta), (r - d) * cos(beta), l * randomf(-0.5, 0.5))
	P = C + capsule:get_rotation() * P
	if abs(capsule:point_depth(P) - d) >= TOL then
		FAILED(5)
	end
...
local function test_plane_point_depth()
	...
	if abs(plane:point_depth(P)) >= TOL then
		FAILED(1)
	end
	-- test the depth of a random point
	local x = randomf(-0.5, 5)
	local y = randomf(-0.5, 5)
	local z = randomf(-0.5, 5)
	...
	if abs(plane:point_depth(P) + z) >= TOL then
		FAILED(2)
	end
	...
	if abs(plane:point_depth(P) - 1) >= TOL then
		FAILED(3)
	end
end
...
local function test_ray_and_sphere()
	...
	if ode.collide(ray, sphere, 1) then
		FAILED(1)
	end
	...
	repeat
		dir = randomdir()
	until dir * n > 0 -- make sure it points away from the sphere
	ray:set(P, dir)
	ray:set_length(100)
	if ode.collide(ray, sphere, 1) then
		FAILED(4)
	...
	if collide then
		...
		if contact.normal * dir > 0 then
			FAILED(8)
		end
		-- check the depth of the contact point
		if abs(sphere:point_depth(contact.position)) > TOL then
			FAILED(9)
		end
	end
	...
	ray:set(P, dir)
	ray:set_length(2)
	if not ode.collide(ray, sphere, 1) then
		FAILED(10)
	end
end
...
local function test_ray_and_box()
	...
	ray:set_length(0)
	local dir = vec3(randomf(-0.5, 0.5) * s.x, randomf(-0.5, 0.5) * s.y, randomf(-0.5, 0.5) * s.z)
	local i = random(1, 3)
	dir[i] = randomsign() * 0.99 * 0.5 * s[i]
	local P = C + box:get_rotation() * dir
	ray:set_position(P)
	ray:set_rotation(randomrot())
	if ode.collide(ray, box, 1) then
		FAILED(1)
	end
	...
	local dir = vec3(randomf(-0.5, 0.5) * s.x, randomf(-0.5, 0.5) * s.y, randomf(-0.5, 0.5) * s.z) * 0.99
	local P = C + box:get_rotation() * dir
	local dir = vec3(randomf(-0.5, 0.5) * s.x, randomf(-0.5, 0.5) * s.y, randomf(-0.5, 0.5) * s.z) * 0.99
	...
	dir[i] = randomsign() * 1.01 * 0.5 * s[i]
	dir = box:get_rotation() * dir
	local P = ray:set(C + 2 * dir, -(dir:normalize()))
	ray:set_length(dir:norm() * 0.99)
	if ode.collide(ray, box, 1) then
		FAILED(5)
	end
	-- test ray from outside to just below surface
	ray:set_length(dir:norm() * 1.01)
	if not ode.collide(ray, box, 1) then
		FAILED(6)
	end
	...
	if collide then
		...
		if abs(box:point_depth(contact.position)) > TOL then
			FAILED(7)
		end
		-- check position of contact point
		local Q = box:get_rotation() * (contact.position - C) -- to box space
		if abs(abs(Q.x) - s.x / 2) > TOL or abs(abs(Q.y) - s.y / 2) > TOL or abs(abs(Q.z) - s.z / 2) > TOL then
			FAILED(8)
		end
		-- check normal signs
		if dir * contact.normal > 9 then
			FAILED(9)
		end
	end
end
...
local function test_ray_and_capsule()
	...
	space:add(capsule)
	-- make a random capsule
	local r = randomf() / 2 + 0.01
	local l = randomf() + 0.01
	capsule:set(r, l)
	local C = randomvec3()
	capsule:set_position(C)
	local rot = randomrot()
	...
	local k = l * randomf(-0.5, 0.5)
	local P = C + 0.99 * k * zaxis + 0.99 * r * randomdir() -- point in the cap
	local Q = C + 0.99 * k * zaxis + 0.99 * r * randomdir()
	ray:set_length((Q - P):norm())
	ray:set(P, (Q - P):normalize())
	if ode.collide(ray, capsule, 1) then
		FAILED(1)
	end
	...
	repeat
		n = randomdir()
	until n * zaxis > 0 -- normal to a cap
	...
	if abs(capsule:point_depth(contacts[1].position)) > TOL then
		FAILED(4)
	end
	...
	local P = C + l / 2 * rot * vec3(0, 0, 1) + 0.99 * r * n -- point just outside the cap
	ray:set(P - 5 * dir, dir)
	ray:set_length(10)
	local collide, contacts = ode.collide(ray, capsule, 1)
	if not collide then
	...
	if collide then
		local contact = contacts[1]
		-- check depth of contact point
		if abs(capsule:point_depth(contact.position)) > TOL then
			FAILED(8)
		end
		-- check normal signs
		if dir * contact.normal > 0 then
			FAILED(9)
		end
	end
end
...
local function test_ray_and_cylinder()
	...
	local P = C + l * randomf(-0.5, 0.5) * zaxis + (r - 0.02) * dir
	ray:set(P, dir)
	ray:set_length(0.01)
	if ode.collide(ray, cylinder, 1) then
		FAILED(1)
	end
	-- test inside ray that just hits the side
	ray:set_length(0.03)
	if not ode.collide(ray, cylinder, 1) then
		FAILED(2)
	end
	...
	local dir = (randomf() * xaxis + randomf() * yaxis):normalize()
	local P = C + (l / 2 - 0.02) * zaxis + (r - 0.02) * dir
	ray:set(P, zaxis)
	ray:set_length(0.01)
	if ode.collide(ray, cylinder, 1) then
		FAILED(4)
	...
	if collide then
		...
		-- there is no cylinder:point_depth() method.
		-- check normal signs
		if zaxis * contact.normal > 0 then
			FAILED(6)
		end
	end
...
local function test_ray_and_plane()
	...
	local P = p * randomf(-0.5, 0.5) + q * randomf(-0.5, 0.5) + n * (d - 0.1)
	ray:set(P, n)
	ray:set_length(0.09)
	if ode.collide(ray, plane, 1) then
		FAILED(1)
	end
	...
	if not collide then
		...
		if abs(plane:point_depth(contacts[1].position)) > TOL then
			FAILED(3)
		end
		if n * contacts[1].normal > 0 then
			FAILED(4)
		end
	end
