...
static const char G_VS_Model[] =
    {
#if GLSL_VERSION > 100
        "uniform mat4 matLights[NUM_LIGHTS];" GLSL_VS_OUT("vec4 fragPosLightSpace[NUM_LIGHTS]")
#endif
...
static const char G_FS_Model[] =
    {
        ...
        "#define NUM_MATERIAL_MAPS"
        " 7\n"
        "#define NUM_MATERIAL_CUBEMAPS"
        " 2\n"
...
static const char G_VS_Depth[] =
    {
        GLSL_VERSION_DEF
            GLSL_VS_IN("vec3 vertexPosition") "uniform mat4 mvp;"
                                              "void main()"
                                              "{"
...
static const char G_FS_Depth[] =
    {
        GLSL_VERSION_DEF
            GLSL_PRECISION("mediump float") "void main(){}"};
...
static const char G_FS_DEBUG_DepthMapDrawing[] =
    {
        ...
        GLSL_FS_OUT_DEF
                    ...
                    "depth = (2.0*near*far)/(far + near - (depth*2.0 - 1.0)*(far - near));" GLSL_FINAL_COLOR("vec4(vec3(depth/far), 1.0)") "}"};
...
static const char G_FS_EquirectangularToCubemap[] =
    {
        ...
        "vec3 color = TEX(equirectangularMap, uv).rgb;" GLSL_FINAL_COLOR("vec4(color, 1.0)") "}"};

static const char G_FS_IrradianceConvolution[] =
    {
        GLSL_VERSION_DEF GLSL_TEXTURE_CUBE_DEF
        ...
        "irradiance = PI * irradiance * (1.0 / float(nrSamples));" GLSL_FINAL_COLOR("vec4(irradiance, 1.0)") "}"};
...
RLG_Context RLG_CreateContext(void)
{
    ...
    if (G_FS_CACHE_Model == NULL)
        TraceLog(LOG_WARNING, "The lighting vertex shader has not been defined.");
    if (G_VS_CACHE_Model == NULL)
        TraceLog(LOG_WARNING, "The lighting fragment shader has not been defined.");
    if (G_VS_CACHE_Depth == NULL)
        TraceLog(LOG_WARNING, "The depth vertex shader has not been defined.");
    if (G_FS_CACHE_Depth == NULL)
        TraceLog(LOG_WARNING, "The depth fragment shader has not been defined.");
